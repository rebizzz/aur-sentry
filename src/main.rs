...
#[derive(Parser)]
#[command(name = "aur-sentry", version, about = "supply-chain security watchdog for the AUR")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn main() -> ExitCode {
    ...
    match cli.command {
        ...
        Commands::Autopilot { repo_root, window_hours, limit } => {
...
fn cmd_scan_file(path: &PathBuf) -> ExitCode {
    let content = match std::fs::read_to_string(path) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("{RED}{ICON_CROSS} can't read {}: {e}{RESET}", path.display());
    ...
    if findings.is_empty() { ExitCode::SUCCESS } else { ExitCode::from(2) }
}
...
fn cmd_scan_pkg(pkgname: &str) -> ExitCode {
    ...
    eprintln!("{BLUE}{ICON_DOWNLOAD} fetching PKGBUILD for {BOLD}{pkgname}{RESET}{BLUE} from AUR...{RESET}");
    let pkgbuild = match client.fetch_pkgbuild(pkgname) {
        Some(c) => c,
        None => {
            eprintln!("{RED}{ICON_CROSS} couldn't find or download PKGBUILD for '{pkgname}'{RESET}");
            return ExitCode::FAILURE;
        }
    };
...
fn cmd_autopilot(repo_root: &PathBuf, window_hours: u64, limit: usize) -> ExitCode {
    ...
    eprintln!("{BLUE}{ICON_PKG} {BOLD}{}{RESET}{BLUE} packages modified in the last {window_hours}h{RESET}", to_scan.len());
    ...
    for pkg in &to_scan {
        ...
        let actionable: Vec<_> = findings.into_iter().filter(|f| f.severity != "INFO").collect();
        ...
        if !actionable.is_empty() {
            ...
            eprintln!(
                ...
                pkg.name, pkg.version, maintainer, actionable.len()
            );
    ...
    eprintln!();
    eprintln!("{CYAN}╭──────────────────────────────────╮{RESET}");
    eprintln!("{CYAN}│{RESET} {ICON_SHIELD}  {BOLD}autopilot complete{RESET}            {CYAN}│{RESET}");
    eprintln!("{CYAN}├──────────────────────────────────┤{RESET}");
    eprintln!("{CYAN}│{RESET}  {ICON_SEARCH}  scanned    {BOLD}{scanned:>6}{RESET}           {CYAN}│{RESET}");
    if flagged > 0 {
        eprintln!("{CYAN}│{RESET}  {RED}{ICON_SKULL}  flagged    {BOLD}{flagged:>6}{RESET}           {CYAN}│{RESET}");
    } else {
        eprintln!("{CYAN}│{RESET}  {GREEN}{ICON_CHECK}  flagged    {BOLD}{flagged:>6}{RESET}           {CYAN}│{RESET}");
    }
    eprintln!("{CYAN}│{RESET}  {ICON_RADAR}  advisories {BOLD}{:>6}{RESET}           {CYAN}│{RESET}", all.len());
    eprintln!("{CYAN}╰──────────────────────────────────╯{RESET}");
...
fn print_findings(findings: &[aur_sentry::scanner::Finding]) {
    ...
    eprintln!();
    eprintln!("  {RED}{ICON_SKULL} {BOLD}{} threat(s) detected{RESET}", findings.len());
    eprintln!();

    for f in findings {
        let color = severity_color(&f.severity);
        let icon = severity_icon(&f.severity);
        eprintln!("  {color}{icon}  [{BOLD}{}{RESET}{color}] {}{RESET}", f.severity, f.rule_id);
        eprintln!("     {}", f.description);
        eprintln!("     {DIM}line {}: {}{RESET}", f.line_number, f.matched_text);
        eprintln!();
    }
}
